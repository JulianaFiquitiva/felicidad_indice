...
if seccion == "Inicio":
    ...
    st.markdown(
        ...
        **Parte 1 — el agente de IA en sí**: cómo funciona, con qué se
        construyó, y cómo se conecta el LLM con el resto del sistema.
    ...
    st.title("Cómo funciona el agente")
    st.caption("Componentes, insights de IA aplicados y cómo se conecta el LLM")
    ...
    st.markdown(
        """
        El agente soporta **múltiples proveedores de LLM intercambiables**
        con detección automática de cuál está configurado — el
    ...
    st.markdown("### Ejemplo real: qué pasa cuando le preguntas algo al agente")
    st.caption('Pregunta de ejemplo: "¿cómo difiere el bienestar por género?"')

    flow_diagram(
        [
            ("1️⃣ Orquestador", "recibe la pregunta"),
            ("2️⃣ LLM interpreta", "decide qué método usar"),
            ("3️⃣ Motor estadístico", "evalúa supuestos y elige la prueba"),
            ("4️⃣ Python calcula", "pandas + scipy ejecutan"),
            ("5️⃣ LLM interpreta<br>el resultado", "lo redacta en lenguaje simple"),
            ("6️⃣ Respuesta<br>completa", "método + supuestos + resultados"),
        ]
    )
    ...
    pasos_ejemplo = [
        (
            "1 · Orquestador",
            "`orchestrator.py`",
            "Recibe la pregunta tal cual la escribiste.",
        ),
        (
            "2 · LLM interpreta",
            "`api_client.py`",
            'Entiende que quieres comparar por género y responde: usar '
            '`smart_comparison` con la columna "Género".',
        ),
        (
            "3 · Motor estadístico evalúa supuestos",
            "`statistical_engine.py`",
            "Shapiro-Wilk → ¿es normal? No. Levene → ¿varianzas iguales? "
            "No. Con esos dos resultados, selecciona **Kruskal-Wallis** "
            "(prueba no paramétrica) en vez de un ANOVA.",
        ),
        (
            "4 · Python ejecuta el cálculo",
            "`pandas` + `scipy`",
            "Carga `03_dataset_IRT_ponderado.xlsx`, corre Kruskal-Wallis y "
            "calcula el p-valor, el eta-cuadrado y las medias por grupo.",
        ),
        (
            "5 · LLM redacta la interpretación",
            "de vuelta en `api_client.py`",
            '"Hay diferencia significativa (p = 0,0048). Los hombres '
            'tienen bienestar más alto (51,15 vs. 49,73)."',
        ),
        (
            "6 · Tú recibes la respuesta",
            "método + supuestos + resultados + interpretación",
            "No solo el número: también qué prueba se usó, qué supuestos "
            "se verificaron, y la explicación en lenguaje simple.",
        ),
    ]
    for titulo, archivo, desc in pasos_ejemplo:
        st.markdown(f"**{titulo}** — {archivo}")
        st.markdown(desc)
        st.write("")

    st.markdown("##### Resumen en 4 pasos")
    flow_diagram(
        [
            ("🧑 Tu pregunta", None),
            ("🧠 LLM entiende<br>qué quieres", None),
            ("🐍 Python calcula<br>la estadística", None),
            ("🧠 LLM te explica<br>el resultado", None),
        ]
    )
